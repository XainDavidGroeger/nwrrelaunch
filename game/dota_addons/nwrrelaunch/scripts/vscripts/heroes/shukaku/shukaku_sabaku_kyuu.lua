...
LinkLuaModifier("modifier_gaara_sabaku_kyuu", "scripts/vscripts/heroes/shukaku/shukaku_sabaku_kyuu.lua", LUA_MODIFIER_MOTION_NONE)

shukaku_sabaku_kyuu = shukaku_sabaku_kyuu or class({})

function shukaku_sabaku_kyuu:Precache(context)
...
function shukaku_sabaku_kyuu:CanBeReflected(bool, target)
    if bool == true then
...
function shukaku_sabaku_kyuu:ProcsMagicStick()
    return true
end

function shukaku_sabaku_kyuu:OnSpellStart()
	if not IsServer() then return end

	self.target = self:GetCursorTarget()

    EmitSoundOn("gaara_prison_cast", self.target)
	EmitSoundOn("gaara_prison_talking", self:GetCaster())
...
function shukaku_sabaku_kyuu:OnChannelFinish(bInterrupted)
	if not IsServer() then return end
